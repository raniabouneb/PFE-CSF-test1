...
export function CertificationsPageContent({ data }: Props) {
  ...
  const isTargetVisible = useCallback(
    (hashId: string) =>
      filtered.some((c) => hashMatchesCardDomId(hashId, `certification-${c.id}`)),
    [filtered],
  )

  useCardScrollTargetHashHighlight({
    targetsReady: allCards.length,
    onUnhideTarget: () => {
      setSearchQuery("")
      setActiveFormations(new Set())
    },
      ...
      <section className="bg-white px-4 py-10 md:py-20 lg:px-8">
        <div className="mx-auto w-full min-w-0 max-w-7xl">
          ...
          <div className="sticky top-16 z-30 -mx-4 mb-10 border-b border-slate-200/70 bg-white/85 px-4 py-3 backdrop-blur sm:top-20 md:-mx-6 md:px-6 lg:-mx-8 lg:px-8">
            <div className="flex min-w-0 flex-wrap items-center justify-center gap-2 py-2 lg:gap-3">
              <FilterPillButton
