...
import {
  loadSiteSearchDefaultSuggestions,
  type SiteSearchDefaultScope,
} from "@/lib/site-search-default-list"
import { isCardScrollTargetId, scrollAndHighlightTarget } from "@/lib/highlight-scroll-target"
...
function defaultScopeForVariant(variant: SiteSearchComboboxVariant): SiteSearchDefaultScope {
  switch (variant) {
    case "certifications-hero":
      return "certifications"
    case "ponctuelle-hero":
      return "ponctuelle"
    case "reconversion-hero":
      return "reconversion"
    default:
      return "all"
  }
...
function sanitizeSearchHrefClient(s: FormationSearchSuggestion): FormationSearchSuggestion {
  ...
  if (k === "ponctuelle" || k === "ponctuelle_module") {
    if (k === "ponctuelle_module") {
      return { ...s, href: sanitizePonctuelleModuleSearchHref(s.href, s.id) }
    }
...
export function SiteSearchCombobox({
  ...
  /** Cache + requêtes en vol par périmètre (certifications, ponctuelle, etc.). */
  const defaultListCacheRef = useRef<Map<SiteSearchDefaultScope, FormationSearchSuggestion[]>>(
    new Map(),
  )
  const defaultListInflightRef = useRef<
    Map<SiteSearchDefaultScope, Promise<FormationSearchSuggestion[]>>
  >(new Map())
  /** Incrémenté à chaque `runFetch` : ignore réponses / finally obsolètes (courses debounce / abort). */
  const fetchGenRef = useRef(0)

  const defaultScope = defaultScopeForVariant(variant)
  ...
  const runFetch = useCallback(async (q: string) => {
    ...
    const scope = defaultScopeForVariant(variant)

    setLoading(true)
    try {
      // Focus + champ vide : liste par défaut selon la page (variant).
      if (!trimmed) {
        searchAbortRef.current?.abort()
        const cachedDefaults = defaultListCacheRef.current.get(scope)
        if (cachedDefaults) {
          if (!stillLeading()) return
          setSuggestions(cachedDefaults)
          return
        }
        const inflight = defaultListInflightRef.current.get(scope)
        if (inflight) {
        ...
        const loadPromise = loadSiteSearchDefaultSuggestions(scope)
        defaultListInflightRef.current.set(scope, loadPromise)
        try {
          const items = await loadPromise
          if (!stillLeading()) return
          defaultListCacheRef.current.set(scope, items)
          setSuggestions(items)
        } finally {
          if (defaultListInflightRef.current.get(scope) === loadPromise) {
            defaultListInflightRef.current.delete(scope)
          }
        }
        return
      }
  ...
  useEffect(() => {
    if (!query.trim() && open) void runFetch("")
  }, [defaultScope, open, query, runFetch])
  ...
  const navigate = (href: string) => {
    ...
    const targetHash = hashIndex >= 0 ? safeHref.slice(hashIndex + 1) : ""
    const isCardHashTarget = isCardScrollTargetId(targetHash)

    // Cas critique UX: clic depuis la liste alors qu'on est déjà sur la page cible.
    // On force hash + scroll/highlight immédiat, sans attendre un cycle de navigation Next.
    if (isCardHashTarget && safeHref.startsWith("/")) {
      const nextPath = safeHref.split("#", 1)[0]?.split("?", 1)[0] || ""
      if (nextPath && nextPath === pathname) {
        window.history.pushState(null, "", safeHref)
        window.dispatchEvent(new Event("hashchange"))
        void scrollAndHighlightTarget(targetHash, { maxWaitMs: 5000 })
        return
      }
    }
    ...
    if (isCardHashTarget) {
      // Navigation inter-page: laisse la route se monter puis pointe la carte ciblée.
      window.setTimeout(() => {
        void scrollAndHighlightTarget(targetHash, { maxWaitMs: 5000 })
      }, 120)
    }
  ...
  const isReconversionHero = variant === "reconversion-hero"
  ...
  const inputClass = cn(
    ...
    (isCertHero || isPonctuelleHero || isReconversionHero) &&
  ...
  const iconClass = cn(
    ...
    (isCertHero || isPonctuelleHero || isReconversionHero) && "text-[#50A3CC]",
  )
