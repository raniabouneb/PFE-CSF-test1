...
export function ValidationCenterBoard() {
  ...
  const [board, setBoard] = useState<BoardState>({ queue: [], selectedId: null, loading: true })
  const { queue, selectedId, loading } = board
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [items, st] = await Promise.all([fetchValidationQueue(), fetchValidationStats()])
      setBoard({ queue: items, selectedId: items[0]?.id ?? null, loading: false })
      setStats(st)
    } catch {
      setBoard({ queue: [], selectedId: null, loading: false })
  ...
  const removeFromQueue = useCallback((id: number) => {
    setBoard((b) => {
      const next = b.queue.filter((q) => q.id !== id)
      let sel = b.selectedId
      if (sel === id || !next.some((q) => q.id === sel)) {
        sel = next[0]?.id ?? null
      }
      return { queue: next, selectedId: sel, loading: false }
    })
  }, [])
  ...
  const pendingCertificates = useMemo(() => queue.filter((q) => q.kind === "certificate").length, [queue])
  const pendingRecommendations = useMemo(() => queue.filter((q) => q.kind === "recommendation").length, [queue])
              ...
              {queue.map((item) => (
                <li key={item.id}>
                  ...
                  >
                    ...
                    <span className="truncate text-xs text-neutral-600">{item.scopeLabel}</span>
                    {item.submittedAt && (
                ...
                <div className="mx-auto flex min-h-0 w-full max-w-2xl flex-1 flex-col overflow-hidden rounded-lg border border-neutral-200 bg-white shadow-md">
                  <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
                    {selected.kind === "certificate" ? (
                      <CertificatePreview doc={selected} />
                    ) : (
