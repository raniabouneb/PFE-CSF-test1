...
export default function CertificationsSummaryCard({
  ...
  successRate = 72,
  blocked = 4,
}: CertificationsSummaryCardProps) {
  return (
    <div className="rounded-xl  bg-white p-6  md:p-8">
      ...
      <div className="mt-6 grid grid-cols-2 gap-4">
        <MetricBox label="Prêtes à imprimer" value={String(readyToPrint)} hint="PDF disponibles" />
        <MetricBox label="Débloquées" value={String(unlocked)} hint="Passage autorisé" />
        <SuccessRateDonutBox label="Réussite globale" percent={successRate} hint="Taux calculé sur vos tentatives" />
        <MetricBox label="Bloquées" value={String(blocked)} hint="Finir le module lié" />
      </div>
    </div>
  );
}
