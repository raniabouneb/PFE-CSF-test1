...
def _map_pack_modules(modules: list[ReconversionPackModule]) -> list[dict[str, Any]]:
...
def _map_pack(p: ReconversionPack, modules: list[ReconversionPackModule]) -> dict[str, Any]:
    return {
        ...
        "modules": _map_pack_modules(modules),
    }


def _map_reconversion_pack_modules_for_card(
    pack: ReconversionPack, modules: list[ReconversionPackModule]
) -> list[dict[str, Any]]:
    """
    ...
    """
    raw_pack_img = (pack.image_url or "").strip() or "/images/hero-formation.png"
    out: list[dict[str, Any]] = []
    for m in sorted(modules, key=lambda x: (x.sort_order, x.id)):
        kind = "project" if m.module_kind == "project" else "course"
...
def load_reconversion_topic_from_db(
    ...
    for p in pack_rows:
        ...
        packs_out.append((p.pack_kind, _map_pack(p, mods)))
    ...
    if gold is not None:
        gold_modules_payload = _map_reconversion_pack_modules_for_card(
            gold, by_pack.get(gold.id, [])
        )

    bg = (topic.hero_background_image or "").strip()
    hero: dict[str, Any] = {
