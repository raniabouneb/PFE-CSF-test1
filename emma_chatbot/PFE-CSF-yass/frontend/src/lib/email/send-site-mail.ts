...
/** Destinataire des demandes Pôle Conseil. */
export function conseilContactRecipients(): string {
  const raw = process.env.CONSEIL_DEMANDE_TO?.trim() || "formation@csfgroupe.tn"
  return raw
    .split(/[,;]/)
    .map((e) => e.trim())
    .filter(Boolean)
...
/** Destinataire des demandes Pôle Solution. */
export function solutionContactRecipients(): string {
  const raw = process.env.SOLUTION_DEMANDE_TO?.trim() || "formation@csfgroupe.tn"
  return raw
    .split(/[,;]/)
    .map((e) => e.trim())
    .filter(Boolean)
...
export async function sendSiteMail(input: {
  ...
  const to = input.to?.trim() || siteContactRecipients()
  if (!to) {
