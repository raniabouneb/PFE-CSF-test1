...
import { X, Users, Calendar, Clock, Download, UserPlus, Key, Gift, Check } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Checkbox } from "@/components/ui/checkbox"
import { APPRENANT_MOCK_GROUPS } from "@/lib/admin/apprenants-groups-mock"
...
export default function GroupDetailsModal({ groupId, onClose, onStudentSelect }: GroupDetailsModalProps) {
  const [activeTab, setActiveTab] = useState<"sessions" | "presences" | "actions">("sessions")
  const [attendance, setAttendance] = useState(mockAttendance)

  const groupMeta = APPRENANT_MOCK_GROUPS.find((g) => g.id === groupId)
  const groupTitle = groupMeta?.name ?? `Groupe ${groupId}`
  const groupSubtitle = groupMeta
    ? `${groupMeta.formation} • ${groupMeta.nombreApprenants} apprenants`
    : ""
