...
function recipientEmails(category: "reconversion" | "ponctuelle" | "certification"): string {
  const raw =
    category === "certification"
      ? process.env.CERTIFICATION_DEMANDE_TO?.trim() || "formation@csfgroupe.tn"
...
export async function POST(req: Request) {
  ...
  const { requestKind, category, trackName, formationTitle, packType, packTag, fullName, email, phone, comment } =
    parsed.data

  const to = recipientEmails(category)
  if (!to) {
...
${safe.comment ? `<tr><td style="padding:6px 12px 6px 0;vertical-align:top"><strong>Commentaire</strong></td><td style="padding:6px 0">${safe.comment}</td></tr>` : ""}
</table>
</body></html>`
