...
export function scrollToElementById(id: string) {
  ...
  void scrollAndHighlightTarget(id)
}
...
const PROGRAM_CARD_PACK_DIR = "/images/pack"

function resolveProgramPackImage(src: string | null | undefined): string {
  const normalized = normalizeImageSrc(src)
  if (!normalized) return `${PROGRAM_CARD_PACK_DIR}/framer1.png`
  if (normalized.startsWith(`${PROGRAM_CARD_PACK_DIR}/`)) return normalized

  const filename = normalized.split(/[\\/]/).pop()?.trim()
  if (!filename) return `${PROGRAM_CARD_PACK_DIR}/framer1.png`

  return `${PROGRAM_CARD_PACK_DIR}/${filename}`
}

const PACK_IMAGES = [
  `${PROGRAM_CARD_PACK_DIR}/framer1.png`,
  `${PROGRAM_CARD_PACK_DIR}/framer2.png`,
  `${PROGRAM_CARD_PACK_DIR}/carte1.jpeg`,
  `${PROGRAM_CARD_PACK_DIR}/aziz.jpeg`,
  `${PROGRAM_CARD_PACK_DIR}/image6.jpg`,
] as const

const STYLE_PRIMARY_PACK_IMAGE: Record<StyleKey, string> = {
  basic: `${PROGRAM_CARD_PACK_DIR}/framer1.png`,
  optimum: `${PROGRAM_CARD_PACK_DIR}/framer2.png`,
  silver: `${PROGRAM_CARD_PACK_DIR}/aziz.jpeg`,
  gold: `${PROGRAM_CARD_PACK_DIR}/carte1.jpeg`,
  microcontroller: `${PROGRAM_CARD_PACK_DIR}/aziz.jpeg`,
  processor: `${PROGRAM_CARD_PACK_DIR}/image6.jpg`,
}
...
function ProgramCardHeroImageColumn({
  ...
  }>(() => ({ from: resolveProgramPackImage(primarySrc), to: resolveProgramPackImage(primarySrc), phase: "idle" }))
  const [dominantMotionInstant, setDominantMotionInstant] = useState(false)

  const normalizedPrimary = useMemo(() => resolveProgramPackImage(primarySrc), [primarySrc])

  const pool = useMemo(() => {
    const out: string[] = []
    const push = (s: string) => {
      const v = resolveProgramPackImage(s)
      if (!v) return
      if (!out.includes(v)) out.push(v)
    }
    push(normalizedPrimary)
    for (const s of PACK_IMAGES) push(s)
    return out.length > 0 ? out : [`${PROGRAM_CARD_PACK_DIR}/framer1.png`]
...
export function ProgramCard({
  ...
  const effectivePrimaryImage = STYLE_PRIMARY_PACK_IMAGE[sk] ?? resolveProgramPackImage(imageUrl)
    ...
    <div className="border-2 border-[#0A566E] rounded-3xl p-6 md:p-8 bg-white">
      ...
      <div className="grid grid-cols-1 items-stretch gap-6 md:gap-8 mb-8 lg:grid-cols-2">
        ...
        <ProgramCardHeroImageColumn
          primarySrc={effectivePrimaryImage}
          styleKey={sk}
          title={title}
