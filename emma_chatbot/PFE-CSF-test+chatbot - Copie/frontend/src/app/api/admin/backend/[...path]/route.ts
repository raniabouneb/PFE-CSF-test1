...
async function forwardAdmin(
  ...
  const target = `${getBackendUrl()}/api/v1/admin/${subpath}${qs ? `?${qs}` : ""}`
  ...
  try {
    ...
    // Uvicorn peut resetter la connexion pendant un hot-reload.
...
export async function GET(req: NextRequest, ctx: RouteCtx) {
  ...
  return forwardAdmin(req, "GET", path ?? [])
}

export async function POST(req: NextRequest, ctx: RouteCtx) {
  const { path } = await ctx.params
  return forwardAdmin(req, "POST", path ?? [])
}

export async function PATCH(req: NextRequest, ctx: RouteCtx) {
  const { path } = await ctx.params
  return forwardAdmin(req, "PATCH", path ?? [])
}

export async function DELETE(req: NextRequest, ctx: RouteCtx) {
  const { path } = await ctx.params
  return forwardAdmin(req, "DELETE", path ?? [])
}
