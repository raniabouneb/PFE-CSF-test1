...
def _recompute_member_progress(db: Session, member: ApprenantGroupMember) -> None:
    ...
    for ref in newly_completed_refs:
        try:
            trigger_validation_documents_for_module(db, int(member.id), ref)
            logger.info("Validation documents triggered for member=%s module=%s", member.id, ref)
        except Exception:
            logger.exception("Failed to trigger validation for member=%s module=%s", member.id, ref)
