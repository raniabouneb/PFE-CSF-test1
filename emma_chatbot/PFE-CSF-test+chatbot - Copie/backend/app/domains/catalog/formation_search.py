...
_PONCTUELLE_FORMATION_SUFFIX = "Formation Par Module"
...
def _formation_search_suggestions_impl(
    ...
    if use_trgm:
        ...
        subtitle_score_sql = _trgm_pair_score_sql(
            "COALESCE(ponctuelle_formation.hero_subtitle, '')"
        )
        mod_trgm_order = text(
            "GREATEST("
            f"{_trgm_pair_score_sql('ponctuelle_module.title')}, "
            f"{_trgm_pair_score_sql('ponctuelle_formation.hero_title')}, "
            f"{subtitle_score_sql}"
            ") DESC NULLS LAST"
        ).bindparams(bq_q)
        mod_rows = (
            db.query(PonctuelleModule, PonctuelleFormation)
    ...
    if use_trgm:
        ...
        rc_topic_subtitle_score_sql = _trgm_pair_score_sql(
            "COALESCE(reconversion_topic.hero_subtitle, '')"
        )
        rc_mod_order = text(
            "GREATEST("
            f"{_trgm_pair_score_sql('reconversion_pack_module.title')}, "
            f"{_trgm_pair_score_sql('reconversion_topic.hero_title')}, "
            f"{rc_topic_subtitle_score_sql}"
            ") DESC NULLS LAST"
        ).bindparams(bq_q)
        rc_mod_rows = (
