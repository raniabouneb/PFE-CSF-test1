...
export function ReconversionCard({ title, description, imageUrl, showHover = false, onDevisClick }: ReconversionCardProps) {
  return (
    <div className="relative rounded-2xl overflow-hidden shadow-lg group cursor-pointer md:row-span-2 h-[300px] md:h-full">
      {/* Background Image */}
      <Image
        src={imageUrl}
        alt={title}
      ...
      {/* Default State - Glassmorphism Text Block at bottom with padding on all sides */}
      <div className={`absolute bottom-4 left-4 right-4 bg-white/20 backdrop-blur-md p-4 rounded-xl border border-white/30 transition-opacity duration-300 ${showHover ? "group-hover:opacity-0" : ""}`}>
        <h3 className="font-bold text-base md:text-lg mb-1 text-white drop-shadow-sm">
          {title}
        </h3>
        <p className="text-xs md:text-sm leading-relaxed text-white/90 drop-shadow-sm">
          {description}
        </p>
      </div>
      ...
      {showHover && (
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <div className="bg-white/20 backdrop-blur-md p-6 rounded-2xl border border-white/30 mx-6 max-w-[320px]">
