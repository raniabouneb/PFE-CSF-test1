...
/** Profil complet (API) — à utiliser dans dashboard/admin ; dédupliqué par requête via cache(). */
export const getCurrentUser = cache(async function getCurrentUser(): Promise<ProfileUser | null> {
  ...
  // La base (auth/me) fait foi ; le JWT peut être obsolète après changement de rôle staff.
