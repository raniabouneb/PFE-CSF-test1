...
export type CertificationAttributionItem = {
  rowKey: string
  enrollmentId: number | null
  validationDocId: number | null
...
async function fetchWithTimeout(input: string, init?: RequestInit, timeoutMs = 15000): Promise<Response> {
  ...
  try {
    ...
    if (err instanceof Error && err.name === "AbortError") {
      throw new Error("Le serveur met trop de temps à répondre. Vérifiez que le backend est démarré.")
    }
    throw err
  } finally {
    clearTimeout(timer)
...
export async function fetchValidationQueue(): Promise<ValidationQueueItem[]> {
  const res = await fetchWithTimeout(`${PROXY}/queue`, { credentials: "include", cache: "no-store" })
  const data = await readJson<{ items: ValidationQueueItem[] }>(res)
  return data.items
}

export async function fetchValidationAttributionList(): Promise<CertificationAttributionItem[]> {
  const res = await fetchWithTimeout(`${PROXY}/attribution-list`, { credentials: "include", cache: "no-store" })
  const data = await readJson<{ items: CertificationAttributionItem[] }>(res)
  return data.items
}

export async function fetchValidationStats(): Promise<ValidationStats> {
  const res = await fetchWithTimeout(`${PROXY}/stats`, { credentials: "include", cache: "no-store" })
...
export async function validateDocument(docId: number): Promise<{ ok: boolean; pdfPath: string | null }> {
  const res = await fetchWithTimeout(`${PROXY}/queue/${docId}/validate`, {
    method: "POST",
    credentials: "include",
  })
...
export async function rejectDocument(docId: number, reason: string): Promise<{ ok: boolean }> {
  const res = await fetchWithTimeout(`${PROXY}/queue/${docId}/reject`, {
    method: "POST",
    credentials: "include",
...
export async function recalculateDocument(docId: number): Promise<ValidationQueueItem> {
  const res = await fetchWithTimeout(`${PROXY}/queue/${docId}/recalculate`, {
    method: "POST",
    credentials: "include",
  })
