...
export async function fetchPonctuelleFormationBySlug(
  ...
  if (direct) {
    return mergeTopicCardHeroImage(direct, "ponctuelle")
  }
  ...
  const data = await fetchPublicApi<PonctuelleFormationPageData>(
    `/api/v1/ponctuelle/formations/${encodeURIComponent(match.slug)}`,
  )
  if (!data) return null
  return mergeTopicCardHeroImage(data, "ponctuelle")
}
