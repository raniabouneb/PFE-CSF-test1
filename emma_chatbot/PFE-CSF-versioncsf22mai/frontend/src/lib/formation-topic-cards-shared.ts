...
const PONCTUELLE_CATALOG_HREF = "/formations-ponctuelles"
...
/** Suggestions `kind: ponctuelle_module` : toujours vers le catalogue global + ancre module. */
export function sanitizePonctuelleModuleSearchHref(
  href: string,
  fallbackId?: string | number,
): string {
  const h = (href ?? "").trim()
  const hashMatch = /#ponctuelle-module-(\d+)/.exec(h)
  const fallbackMatch = /(\d+)/.exec(String(fallbackId ?? ""))
  const moduleId = hashMatch?.[1] || fallbackMatch?.[1] || ""
  if (!moduleId) return PONCTUELLE_CATALOG_HREF
  return `${PONCTUELLE_CATALOG_HREF}#ponctuelle-module-${moduleId}`
}
