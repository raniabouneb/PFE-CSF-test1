...
import { PonctuelleModulesPicker } from "./ponctuelle-modules-picker"
...
const sectionsData = {
  ...
  ponctuelle: {
    title: PONCTUELLE_OFFER_LABEL,
    description:
      "Montez en compétences sur un sujet précis. Accédez à notre portfolio complet de modules experts.",
...
interface FormatsSectionProps {
  /** Si défini, remplace les listes statiques pour reconversion (par section non vide). */
  topicCardsFromDb?: FormationTopicCardsFromDb | null
  initialPonctuelleModules?: PonctuelleModuleListItem[]
}

export function FormatsSection({
  topicCardsFromDb = null,
  initialPonctuelleModules = [],
}: FormatsSectionProps) {
  ...
  const topicCards = useMemo((): FormationTopicCardsFromDb | null => {
    ...
    if (fromDb && (fromDb.reconversion?.length ?? 0) > 0) {
      return fromDb
    }
  ...
  const renderExpandedContent = (section: FormationOpenSection) => {
    ...
    if (section === "ponctuelle") {
      return (
        <>
          <PonctuelleModulesPicker initialModules={initialPonctuelleModules} />
