...
from app.domains.admin.validation.service import list_attribution_candidates
from app.domains.validation.pdf_generator import (
    generate_certificate_pdf,
    generate_recommendation_pdf,
)
from app.domains.validation.certificate_templates import resolve_certificate_template_id
...
def _doc_to_queue_item(doc: ValidationDocument, member: ApprenantGroupMember, group: ApprenantGroup) -> sch.ValidationQueueItem:
    return sch.ValidationQueueItem(
        ...
        certificateTemplateId=resolve_certificate_template_id(
...
# ── Attribution list (all enrolled) ─────────────────────────────────

@router.get("/attribution-list", response_model=sch.CertificationAttributionResponse)
def validation_attribution_list(_staff: StaffUser, db: Db) -> sch.CertificationAttributionResponse:
    items = list_attribution_candidates(db, auto_prepare=True)
    return sch.CertificationAttributionResponse(
        items=[sch.CertificationAttributionItem.model_validate(item) for item in items]
    )
