...
class ValidationQueueItem(BaseModel):
    ...
    scopeRef: str
    scopeLabel: str
    certificateTemplateId: str = "default"
    presencePercent: int
    absenceCount: int
...
AttributionStatus = Literal["not_ready", "ready_no_doc", "pending_admin", "validated", "rejected"]


class CertificationAttributionItem(BaseModel):
    rowKey: str
    enrollmentId: int | None = None
    validationDocId: int | None = None
    ...
    progressStatus: str
    totalSessions: int = 0
    presenceEligible: bool = False
    attributionStatus: AttributionStatus
    kind: ValidationDocKind = "certificate"
    scopeLabel: str
    certificateTemplateId: str = "default"
    ...
    absenceSessions: list[AbsenceSessionItem] = Field(default_factory=list)
    testScorePercent: int | None = None
    systemResult: SystemResult = "failure"
    systemReason: str = ""
    status: str
    submittedAt: datetime | None = None


class CertificationAttributionResponse(BaseModel):
    items: list[CertificationAttributionItem]
